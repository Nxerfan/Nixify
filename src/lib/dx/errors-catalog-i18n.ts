...
export const FA_TRANSLATIONS: Record<string, {
  ...
  validation_failed: {
    ...
    causes: ["فیلد مورد نیاز موجود نیست (ایمیل، کد، رمز عبور)", "فرمت ایمیل نامعتبر", "کد OTP دقیقاً ۶ رقم نیست", "رمز عبور کوتاه‌تر از ۸ کاراکتر است"],
  ...
  unauthorized: {
    ...
    causes: ["هدر Authorization موجود نیست", "کلید API فرمت نامعتبر دارد (باید با mg_live_ یا mg_test_ شروع شود)", "کلید API وجود ندارد"],
    fixes: ["یک کلید API در داشبورد در /dashboard/api-keys ایجاد کنید", "آن را به صورت: Authorization: Bearer mg_live_xxx ارسال کنید"],
  },
  key_revoked: {
  ...
  rate_limited: {
    title: "محدودیت نرخ",
    description: "تعداد درخواست‌ها در پنجره زمانی بیش از حد بوده است. کد rate_limited شامل سه محدودکننده مستقل است: هر ایمیل (۳/دقیقه، ۱۰/ساعت)، هر IP (ارسال + تأیید)، و نرخ درخواست API هر دقیقه پلن. هر ۴۲۹ با این کد شامل هدر Retry-After نیست — برای الگوی دو هدر به راه‌حل‌ها مراجعه کنید.",
    causes: ["بیش از ۳ ارسال OTP در دقیقه برای هر ایمیل", "بیش از ۱۰ ارسال OTP در ساعت برای هر ایمیل", "محدودیت نرخ سطح IP فراتر رفته (ارسال یا تأیید)", "محدودیت نرخ درخواست API هر دقیقه پلن فراتر رفته (موتور حقوق کاربر)"],
    fixes: ["۴۲۹های IP/ایمیل: به مدت هدر Retry-After (ثانیه) قبل از تلاش مجدد صبر کنید؛ ۴۲۹های سطح ایمیل additionally شامل X-RateLimit-Limit/Remaining/Reset هستند", "۴۲۹های نرخ پلن (بدون Retry-After): برای X-RateLimit-Reset صبر کنید و X-Quota-Remaining را بررسی کنید؛ backoff نمایی پیاده‌سازی کنید", "فرکانس درخواست را کاهش دهید یا به پلنی با نرخ بالاتر در دقیقه ارتقا دهید"],
  },
  locked: {
    title: "قفل شده",
    ...
    fixes: ["۱۵ دقیقه برای انقضای قفل هر کد صبر کنید", "۳۰ دقیقه برای انقضای قفل حساب صبر کنید", "اگر قفل پس از پایان دوره انتظار ادامه یافت، با شناسه درخواست با پشتیبانی تماس بگیرید"],
  },
  code_mismatch: {
    title: "عدم تطابق کد",
  ...
  quota_exceeded: {
    ...
    description: "سهمیه ماهانه API_MESSAGES پلن شما تمام شده است. این سهمیه توسط هر درخواست احراز هویت v1 API مصرف می‌شود — نه فقط نقاط انتهایی OTP (broadcasts، suppressions، groups، events، deliveries و تمام مسیرهای دیگر v1 نیز آن را مصرف می‌کنند). این سهمیه از محدودیت‌های هر ایمیل و هر IP جدا است.",
    causes: ["پلن دارنده کلید تمام سهمیه ماهانه API_MESSAGES خود را مصرف کرده", "توجه: کلیدهای mg_test_ (سندباکس) متعلق به کاربر نیز این سهمیه را مصرف می‌کنند — حالت سندباکس ارسال ایمیل واقعی و محدودیت نرخ هر ایمیل را نادیده می‌گیرد، اما سهمیه پلن را خیر"],
    fixes: ["برای بازنشانی سهمیه در چرخه بعدی صورتحساب صبر کنید", "به پلن بالاتری با سهمیه API_MESSAGES بیشتر ارتقا دهید", "حجم درخواست‌ها را با batching یا caching در صورت امکان کاهش دهید"],
  },
  ...
  internal_error: {
    ...
    fixes: ["با backoff نمایی تلاش مجدد کنید", "با شناسه درخواست از بدنه پاسخ یا هدر X-Request-Id با پشتیبانی تماس بگیرید"],
  },
};
