...
import {
  ...
  BookOpen, Compass, Sparkles, Search,
} from "lucide-react";
 ...
 *
 * Structure:
 *   A. Hero — title, description, total guides + learning time
...
export function GuideLanding({ guides, categories }: GuideLandingProps): React.ReactElement {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-24 pt-28 sm:px-6" dir={dir}>
      {/* A. Hero */}
      <motion.div
      ...
      >
        ...
        <p className="max-w-2xl text-base text-gray-400 sm:text-lg">{copy.description}</p>
          ...
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex-1 space-y-3">
                ...
                <h2 className="text-2xl font-bold text-gray-100">{localizeTitle(featured, locale)}</h2>
                <p className="max-w-xl text-sm text-gray-400">{localizeDescription(featured, locale)}</p>
            ...
            return (
              <div key={cat.id}>
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">({catGuides.length})</span>
                </div>
              ...
              return (
                <React.Fragment key={step.slug}>
                  <Link
                    ...
                    className="group flex items-center gap-2 rounded-xl border border-gray-700/60 bg-gray-950/60 px-4 py-3 transition hover:border-emerald-500/30 hover:bg-gray-900/40"
                  >
                    <Icon className="h-4 w-4 text-emerald-400" />
                    <div>
                      <p className="text-sm font-medium text-gray-200">{localizeTitle(guide, locale)}</p>
                      <p className="text-[10px] text-gray-500">{step.reason}</p>
                    </div>
                  </Link>
          ...
          {copy.tasks.map((task, i) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] text-gray-500">{localizeTitle(publishedGuides.find((g) => g.slug === task.slug)!, locale)}</p>
              </div>
      ...
      <section className="mb-12">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          >
            <BookOpen className="mb-2 h-5 w-5 text-gray-400" />
...
function GuideCard({
  ...
  return (
    ...
    >
      <Link
        href={`/guide/${guide.slug}`}
        className="group flex h-full flex-col rounded-xl border border-gray-800/60 bg-gray-950/40 p-5 transition hover:border-emerald-500/30 hover:bg-gray-950/60"
      >
        <div className="mb-3 flex items-center gap-2">
        ...
        <p className="mb-3 flex-1 text-xs text-gray-400">{localizeDescription(guide, locale)}</p>
