...
export function EmailChangesExplainer({
  ...
  return (
    <article className="rounded-2xl border border-border bg-muted/40 p-5 sm:p-7" dir={dir}>
          ...
          <tbody>
            {copy.mappings.map((m, i) => (
              ...
              >
                ...
                <td className="px-3 py-2.5">
                  <span className="inline-flex items-center gap-1.5 rounded-md border border-border bg-muted/60 px-1.5 py-0.5 text-[10px] text-muted-foreground">
                    <Ltr>{m.before}</Ltr>
                  </span>
                </td>
                <td className="px-3 py-2.5">
      ...
      <p className="mt-4 border-t border-border pt-3 text-xs text-muted-foreground">{copy.footnote}</p>
    </article>
  );
}
