...
export function EmailChangesExplainer({
  ...
  return (
    <article className="rounded-2xl border border-gray-800/60 bg-gray-950/40 p-5 sm:p-7" dir={dir}>
          ...
          <tbody>
            {copy.mappings.map((m, i) => (
              ...
              >
                ...
                <td className="px-3 py-2.5">
                  <span className="inline-flex items-center gap-1.5 rounded-md border border-gray-700/60 bg-gray-900/60 px-1.5 py-0.5 text-[10px] text-gray-400">
                    <Ltr>{m.before}</Ltr>
                  </span>
                </td>
                <td className="px-3 py-2.5">
      ...
      <p className="mt-4 border-t border-gray-800/60 pt-3 text-xs text-gray-500">{copy.footnote}</p>
    </article>
  );
}
