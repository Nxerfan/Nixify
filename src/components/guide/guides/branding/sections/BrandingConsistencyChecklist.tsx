...
import type { ConsistencyChecklistCopy } from "@/lib/guide/content/guides/branding-types";

/**
 * Brand Consistency Checklist — creative section.
 *
 * A practical checklist of brand-consistency checks the user should run
 * before clicking "Activate" on a saved theme. Each item is annotated with
 * a hint explaining why it matters. The bottom warning card reminds users
 * not to mix active themes across purposes.
 *
 ...
 */
export function BrandingConsistencyChecklist({
  copy,
}: {
  copy: ConsistencyChecklistCopy;
}): React.ReactNode {
  const { dir } = useLocale();
