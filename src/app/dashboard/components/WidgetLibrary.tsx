    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div
            className="fixed right-0 top-0 z-50 h-full w-80 overflow-y-auto border-l border-gray-800/50 p-5"
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-gray-100">Widget Library</h2>
              <button
                onClick={onClose}
                className="rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-gray-800/50 hover:text-gray-200"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-2">
              {allWidgets.map((widget, i) => (
                <motion.div
                  key={widget.id}
                  className="flex items-center justify-between rounded-lg border border-gray-800/40 bg-gray-950/40 p-3"
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: i * 0.05, duration: 0.3, ease: EASE }}
                >
                  <div className="min-w-0 flex-1 pr-3">
                    <p className="truncate text-sm font-medium text-gray-200">{widget.name}</p>
                    <p className="truncate text-xs text-gray-600">{widget.description}</p>
                  </div>
                  <button
