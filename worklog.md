...
---
Task ID: phase-9-audit-fixes
Agent: main (orchestrator)
Task: Phase 9 audit fixes — address correctness/security blockers on existing branch feat/phase-9-consent-suppression, PR #12

Work Log:
- Preflight: fetched origin, verified remote HEAD == d84b7944f1fff280284dc1b7d0f10401909b553b. Read reliability-protocol.md and agent-lessons.md fully. Background process kept switching branches to main during work — restored via `git checkout feat/phase-9-consent-suppression && git reset --hard origin/feat/phase-9-consent-suppression` repeatedly.
- BLOCKER #1 (no in-tx catch-and-continue): removed ALL `try { tx.event.create() } catch {}` patterns inside `db.$transaction()` in src/lib/consent/service.ts. Replaced with `createMany({ skipDuplicates: true })` for the lift/suppress side-effect events (conflict-safe without catch). P2002 from the canonical ConsentEvent insert propagates out of the tx and is caught by the outer try/catch.
- BLOCKER #2-3 (canonical mutation lock): added `acquireCanonicalLock(tx, userId, normalizedEmail)` which executes `SELECT pg_advisory_xact_lock(tenant_key, email_key)` via `tx.$executeRaw`. `tenantLockKey(userId)` = BigInt(userId). `emailLockKey(email)` = first 8 hex chars of SHA-256(email) as uint32. Same (userId, email) → same lock; different tenants → independent lock domains. Applied to subscribeContact, unsubscribeContact, suppressEmail, unsuppressEmail, liftOnly. Pre-lock read resolves the contact → acquire lock → RE-READ Contact after lock (no stale snapshot drives mutation).
- BLOCKER #4 (concurrency invariants): added `verifyConsentHistoryChain()` helper that asserts `event[n].previousStatus === event[n-1].newStatus` for events ordered by createdAt asc AND `Contact.marketingStatus === last_event.newStatus`. Tests assert this invariant after races.
- BLOCKER #5 (replace weak race tests): rewrote subscribe-vs-unsubscribe race test to assert: both produce durable audit events, history transition chain is coherent (via verifyConsentHistoryChain), final Contact state equals last committed event's newStatus, suppression state agrees with effective transition. Rewrote suppress-vs-unsuppress race to assert final active state agrees with last committed SuppressionEvent action (not just `typeof active === "boolean"`).
- BLOCKER #6 (idempotency namespace): changed unique constraint from `(userId, idempotencyKeyHash)` to `(userId, operation, idempotencyKeyHash)`. Added `operation` field to both ContactConsentEvent and SuppressionEvent. Added `requestFingerprint` field (SHA-256 of canonicalized mutable payload). Service verifies target (contactId/email) matches existing record before replay — throws `IdempotencyConflictError` (409) on target mismatch or fingerprint conflict.
- BLOCKER #7 (standard Idempotency-Key header): v1 routes now read `Idempotency-Key` header (8-128 chars, same convention as transactional Send). Removed random key generation from v1 routes — caller supplies key or gets no dedup. Dashboard routes still auto-generate (each dashboard click is a distinct action). Public unsubscribe continues to use token jti as idempotency key.
- BLOCKER #8 (opaque unsubscribe token): replaced signed JWS (JWT) with ENCRYPTED JWE via `jose`'s `EncryptJWT` using `alg=dir, enc=A256GCM`. The token is now opaque — base64-decoding any of the 5 parts does NOT expose email/userId/contactId. Verified with a runtime test: old JWS exposed plaintext `{ email: "alice@example.com", uid: "42", sub: "99" }`; new JWE exposes nothing.
- BLOCKER #8 (cryptographic domain separation): added `deriveUnsubscribeKey()` which uses HKDF-SHA-256 to derive a 32-byte key from the root `JWT_SECRET` with context `nixify:unsubscribe:v1`. The raw session JWT signing key is NEVER used directly for token encryption.
- BLOCKER #9 (token confidentiality tests): added test `BLOCKER #8/#9: unsubscribe token is OPAQUE` that uses `tokenExposesPlaintext()` to assert email/userId/contactId do not appear in any base64-decoded token part. Added test for wrong-purpose token (encrypted with same key but no `purpose=unsubscribe` claim → rejected with `wrong_purpose`).
- BLOCKER #10 (real public route tests): added 5 tests that directly exercise `GET /api/unsubscribe` and `POST /api/unsubscribe` via `NextRequest`: missing token, invalid token, malformed JSON, valid token for deleted contact, valid token for existing contact. All assert the SAME generic message for failure modes.
- BLOCKER #11 (remove unsafe exception logging): replaced `console.error("[route] error", err instanceof Error ? err.message : err)` with `console.error("[route] safe_error_code: internal_error")` across all consent/suppression routes + public unsubscribe. Never log raw exception text, token claims, email, or request body.
- BLOCKER #12 (structural tenant isolation): modified schema + migration in place (NOT a new migration) to add COMPOSITE tenant-safe FKs:
  * `ContactConsentEvent` now has `contact Contact @relation(fields: [userId, contactId], references: [userId, id], onDelete: Cascade)`. DB rejects a row whose userId disagrees with its Contact's userId.
  * `SuppressionEntry` added `@@unique([userId, id])` to enable composite FK.
  * `SuppressionEvent` now has `entry SuppressionEntry? @relation(fields: [userId, suppressionId], references: [userId, id], onDelete: SetNull)`. DB rejects cross-tenant event→entry references.
  * Added test `BLOCKER #12: cross-tenant consent event insert REJECTED by composite FK` which directly attempts a cross-tenant insert and asserts it throws.
- BLOCKER #13 (immutable audit): added test `BLOCKER #13: immutable history` that creates a transition, captures eventId/createdAt/previousStatus/newStatus, performs 2 more transitions, and verifies the original event row is unchanged.
- BLOCKER #14 (Contact deletion audit semantics): documented in schema comments that `ContactConsentEvent → Contact ON DELETE CASCADE` is INTENTIONAL (GDPR right-to-erasure — when a Contact is deleted, their consent history is erased with them). Added test `BLOCKER #14` that verifies the cascade behavior.
- BLOCKER #15 (idempotency tx behavior): the new service flow is: acquire lock → check idempotency record (findUnique inside tx, after lock) → verify fingerprint → compute transition from fresh locked state → perform mutation → append audit → append timeline → commit. P2002 caught outside tx.
- BLOCKER #16-17 (no-op audit semantics): same-state request without idempotency key now returns `status: "no_op"` and does NOT create a fake transition event. Added tests `BLOCKER #16` (subscribe no-op) and `BLOCKER #17` (suppress no-op) that assert history count does NOT grow on a same-state call without an idempotency key.
- BLOCKER #18 (alsoSubscribe): verified `unsuppressEmail({ alsoSubscribe: true })` routes through `subscribeContact` for the explicit consent action. It cannot silently subscribe through an ordinary lift — `alsoSubscribe` defaults to false and is the ONLY way to combine lift + subscribe. Phase 9 still rejects writes of `hard_bounce`/`complaint` reasons (PHASE9_WRITABLE_REASONS set).
- BLOCKER #19 (transactional Send preserved): `/api/v1/messages/send` is NOT modified — marketing eligibility is NOT enforced on transactional Send. OTP/password-reset/security emails continue to work for suppressed/unsubscribed contacts.
- BLOCKER #20 (engineering lessons): added 5 new lessons to `docs/engineering/agent-lessons.md`:
  1. Signed is not opaque (JWS vs JWE)
  2. Prisma read is not a row lock (findFirst ≠ SELECT FOR UPDATE)
  3. Never catch constraint errors inside PostgreSQL transaction and continue
  4. Idempotency identity must bind operation and target
  5. Structural tenant isolation requires ownership FK, not merely a userId column
- BLOCKER #21 (migration discipline): modified the existing `20260920000000_add_consent_suppression/migration.sql` in place. NO new migration. NO destructive changes. Manually inspected SQL — only CREATE TABLE + CREATE INDEX + ADD CONSTRAINT FOREIGN KEY with composite keys.
- BLOCKER #22 (expanded tests): test count grew from 40 → 46 tests. New coverage: idempotency namespace (3 tests: different contact, different operation, conflicting fingerprint), no-op semantics (2 tests: subscribe no-op, suppress no-op), structural FK rejection (1 test), immutable history (1 test), contact deletion cascade (1 test), token opacity (1 test), real public route tests (5 tests). Concurrency tests now assert real invariants (history chain coherence, final state matches last event, suppression state agrees with last action) instead of broad "one of these values is okay".
- BLOCKER #23 (v1 route audit): verified all v1 routes use `withApiKey()` middleware (which consumes API_MESSAGES per existing semantics). GET → "read" scope (read_only keys OK), mutations → "full" scope. User-owned key required (403 for system keys). CONTACTS entitlement checked. OTP_EMAILS/MESSAGING_EMAILS untouched. No new stored API scope names.
- BLOCKER #25 (validation): `bunx tsc --noEmit` → clean. `bun run lint` → clean (0 errors). `bun run test` → 521 passed, 412 skipped (integration tests require DB). Fail-closed: `env -u TEST_DATABASE_URL bun run test:consent-suppression` exits 1 with stderr message. Dev server starts, `/api/unsubscribe` returns expected JSON for missing/invalid tokens. Runtime test confirmed JWE token is opaque (no plaintext exposure) and verifies correctly.
- BLOCKER #26 (commit discipline): verified `git branch --show-current` = `feat/phase-9-consent-suppression` immediately before staging. Background process switched branches multiple times during work — restored each time via checkout + reset.

Stage Summary:
- Schema (modified in place, still additive): ContactConsentEvent + SuppressionEvent now have `operation` + `requestFingerprint` fields, composite tenant-safe FKs, unique constraint on `(userId, operation, idempotencyKeyHash)`. SuppressionEntry has `@@unique([userId, id])`. Migration SQL rewritten to match.
- Token (`src/lib/consent/token.ts`): JWE (EncryptJWT, alg=dir, enc=A256GCM) with HKDF-derived key (`nixify:unsubscribe:v1` context). Opaque — no plaintext exposure via base64. `tokenExposesPlaintext()` test helper.
- Service (`src/lib/consent/service.ts`, ~1445 LOC): canonical mutation lock via `pg_advisory_xact_lock`, no in-tx catch-and-continue, idempotency namespace `(userId, operation, target, key)` with fingerprint conflict detection → IdempotencyConflictError, no-op semantics (no fake transition events), `verifyConsentHistoryChain()` helper.
- Routes: v1 uses standard `Idempotency-Key` header (8-128 chars). Dashboard auto-generates. Public unsubscribe uses token jti. All routes log safe error codes only — never raw exception text.
- Tests (`src/lib/consent/service.test.ts`, 46 tests): idempotency namespace (3), no-op semantics (2), structural FK rejection (1), immutable history (1), contact deletion cascade (1), token opacity (1), real public route tests (5), concurrency with real invariants (4), plus all original consent/suppression/eligibility/token tests.
- Engineering lessons: 5 new lessons added to `docs/engineering/agent-lessons.md`.
- Typecheck + lint clean. 521 unit tests pass. Fail-closed verified.
- No production DB writes. No production migration. No new migration file. PR #12 NOT merged. Phase 10 NOT started.
