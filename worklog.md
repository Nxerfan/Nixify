...
---
Task ID: landing-pages
Agent: fullstack-developer
Task: Create two new public landing pages — /email-otp-api and /email-verification-api — as locale-aware (EN/FA) server components on the feat/ai-search-discoverability branch.

Work Log:
- Read the last ~40 lines of worklog.md plus the full /security, /about, /examples, /docs, /blog/[slug], /guide/[section]/not-found, /compare, and /status pages to anchor on the established i18n + design patterns. Also inspected src/lib/i18n/server-locale.ts, src/lib/i18n/Ltr.tsx, src/lib/i18n/locales.ts, src/lib/site/site-url.ts, src/lib/site/public-routes.ts, src/app/api/v1/otp/{send,verify,resend}/route.ts, src/lib/otp/verifier.ts, src/lib/dx/sandbox.ts, and src/app/docs/DocsContent.tsx for endpoint / sandbox / lockout / rate-limit facts.
- Confirmed `PUBLIC_MARKETING_ROUTES` in src/lib/site/public-routes.ts already lists both `/email-otp-api` and `/email-verification-api` — sitemap.ts and llms.txt will pick them up automatically; no edits to discoverability sources were required.
- Confirmed the canonical OTP constants used in the copy: `OTP_TTL_MS = 10 * 60 * 1000` (10-min TTL), `OTP_LOCKOUT_MS = 15 * 60 * 1000` (15-min lockout), `OTP_MAX_ATTEMPTS = 5`. Confirmed the lockout is anchored to `latest.createdAt` (verifier.ts line 456) — i.e. 15 min from OTP *creation*, NOT from the 5th failed attempt. This is the precise semantics surfaced on the verification page.
- Confirmed the sandbox contract: `mg_test_` keys skip the per-email limiter for /send and /resend (issueOtp called with `skipEmailRateLimit: true`) but /verify still enforces `5/min` per email and the per-IP limits (`30/min`, `120/hour`) apply to all keys. The `X-Sandbox-Simulate` header is honored only for dev-environment keys and accepts `rate_limited | locked | smtp_error` for /send and /resend, plus `mismatch | expired | locked` for /verify — matching the documented scenarios.
- Confirmed the two response IDs: `request_id` (API trace, matches `X-Request-Id` header) vs `otp_request_id` (OTP correlation, surfaces as `requestId` in webhook events). Confirmed clients do NOT send either back to /verify — verification is by email + code + purpose.
- Created `src/app/email-otp-api/page.tsx` (server component, no "use client"):
  • `generateMetadata()` resolves the locale via the shared `resolveServerLocale()` helper (same one the root layout uses) and produces locale-aware title/description, `alternates.canonical: "/email-otp-api"`, openGraph.url via `absoluteUrl("/email-otp-api")`, and a Twitter summary_large_image card.
  • The default export is `async function EmailOtpApiPage()` which awaits `resolveServerLocale()`, derives `isFa = locale === "fa"`, computes `dir = LOCALE_HTML_DIR[locale]`, and renders the page with `dir={dir}` on the root wrapper so the Persian variant flips to RTL.
  • All natural-language copy is built through a `t` record of `isFa ? "..." : "..."` conditionals — no separate `fa.ts` dictionary drift. Every technical token (endpoint paths, header names, JSON keys, error codes, environment variable names, HTTP status numbers, code samples) is wrapped in `<Ltr>` (the existing `@/lib/i18n/Ltr` inline span with `dir="ltr"` + `unicode-bidi: isolate`) or rendered inside `<pre dir="ltr">` so it stays LTR inside an RTL paragraph.
  • Sections (in order): Hero → How it works (3-step send → verify → resend lifecycle) → Endpoints (request/response tables + possible error badges for all three endpoints) → OTP properties (6 ASCII digits, 10-min TTL, max 5 attempts, single-use, HMAC-SHA256, plaintext never persisted) → Rate limits (table with all 5 rows + the mg_test_ skip note + the "configurable security defaults may be overridden by deployment" caveat) → Test vs Live keys (mg_test_/mg_live_ cards + X-Sandbox-Simulate split table for send/resend vs verify scenarios) → Webhooks (otp.sent/verified/failed/expired with links to /docs#webhooks and /guide/webhooks) → Architecture (Browser/App → Your backend → Nixify API diagram; "never expose API key in client code" warning) → Code examples in Node.js (fetch), Python (requests), and PHP (cURL), each showing send + verify with the `purpose` parameter and using `PRODUCTION_ORIGIN` for absolute URLs → Request IDs (request_id vs otp_request_id; clients do NOT send either back) → Latency & reliability (verbatim "Nixify does not currently publish a fixed latency SLA. Recent average API request latency is available on the public Status page and is derived from application telemetry." + link to /status) → Cross-links to /docs, /email-verification-api, /pricing, /security, /examples.
  • No fake reviews, no AggregateRating, no customer counts, no uptime %, no SLA claim, no SDK announcement, no hardcoded latency numbers.
- Created `src/app/email-verification-api/page.tsx` (server component, distinct from /email-otp-api):
  • Same `generateMetadata()` + `resolveServerLocale()` + `isFa` + `dir` pattern. Canonical `/email-verification-api`.
  • Focus is on EMAIL OWNERSHIP VERIFICATION — does NOT duplicate the endpoint reference. Sections: Hero (verification-focused subtitle) → Verification flow (signup/login/reset purposes, 3-step issue→verify→resend lifecycle, code-expiry callout) → Security properties (5-attempt brute-force protection, 15-min lockout anchored to OTP creation NOT the 5th attempt, HMAC-SHA256, single-use atomic UPDATE) with a side-by-side `expired` (HTTP 410, time-based, normal state) vs `locked` (HTTP 423, security state, possible brute-force signal) distinction card → Verification state & session (4-step architecture: app calls /send → user enters code → app calls /verify → if verified, app sets emailVerified=true in its OWN db and establishes a session; optional otp.verified webhook for idempotency; explicit "Nixify does not see your user database" caveat) → Webhook events (otp.verified/failed/expired, link to /docs#webhooks) → Error handling (table of code_mismatch/expired/locked/already_used/not_found/rate_limited with HTTP status + per-code UI action) → Rate limits (/verify rows highlighted with emerald-500/5 bg; mg_test_ still enforces 5/min on /verify) → Code examples in Node.js/Python/PHP focused on the verify flow with full error-code mapping switch/dict → Latency (same verbatim wording + link to /status) → Cross-links to /docs, /email-otp-api, /pricing, /security.
  • Both pages use the existing design system: `AmbientBackground` from `@/app/auth/components/AmbientBackground`, `max-w-4xl` content shell with `px-4 pb-24 pt-28 sm:px-6` matching /security + /examples, emerald accent (`text-emerald-600 dark:text-emerald-400`), `rounded-lg border border-border bg-card/60 p-6` cards, tables with `sticky top-0` headers and `max-h-96 overflow-y-auto` for long lists, `text-rose-400` for required-field asterisks and destructive lockout badges, `text-amber-300`/`bg-amber-500/15` for test-key badges and `expired` cards.
- Used `Link` from `next/link` for all internal cross-links (not raw `<a href>`), per Next.js best practice.
- Verified code blocks via static `<pre dir="ltr">` blocks (server-renderable; the client `CodeBlock` component is intentionally avoided because these are pure server components).
- Verified both files compile cleanly: `bun run typecheck` → 0 errors. `bun run lint` → exit code 0, 0 warnings.

Stage Summary:
- Two new public landing pages added on the feat/ai-search-discoverability branch:
  • `src/app/email-otp-api/page.tsx` — server component, EN/FA locale-aware via `isFa` conditionals + `<Ltr>` for technical tokens. Documents the 3-endpoint OTP API with full request/response tables, the rate-limit table, the test-vs-live sandbox split, webhooks, multi-language code examples using `PRODUCTION_ORIGIN`, the request_id vs otp_request_id distinction, and the no-fixed-latency-SLA wording.
  • `src/app/email-verification-api/page.tsx` — server component, distinct from /email-otp-api. Focuses on email ownership verification: signup/login/reset flow, the brute-force lockout semantics (5 attempts, 15-min lockout anchored to OTP creation NOT the 5th attempt, expired vs locked distinction), the verify state machine, verify error catalog, and verify-focused code examples.
- Both pages: canonical URLs already in `PUBLIC_MARKETING_ROUTES` (sitemap.ts/llms.txt will pick them up automatically). `metadata.canonical` set via `alternates.canonical`. `openGraph.url` via `absoluteUrl(path)`. No fake ratings/reviews, no uptime %, no SLA claims, no SDK announcements, no hardcoded latency numbers, no translations of endpoint paths/HTTP methods/JSON keys/code.
- Pre-existing environment note (NOT caused by this task): the dev-server bootstrap script `bun run db:push` fails because `prisma/schema.prisma` declares `provider = "postgresql"` while `.env` sets `DATABASE_URL="file:./db/custom.db"` (SQLite format). This blocks the auto-dev-server from starting; the pages themselves compile cleanly (typecheck + lint both pass). This is a pre-existing schema/env mismatch outside the landing-pages task scope.
